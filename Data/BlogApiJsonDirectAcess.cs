...
namespace Data
{
    public class BlogApiJsonDirectAcess : IBlogApi
    {
        ...
        private void ManageDataPaths()
        {
            ...
            CreateDirectoryIfNotExists($@"{_settings.DataPath}\{_
settings.BlogPostsFolder}");
            CreateDirectoryIfNotExists($@"{_settings.DataPath}\{_
settings.CategoriesFolder}");
            CreateDirectoryIfNotExists($@"{_settings.DataPath}\{_
settings.TagsFolder}");
            CreateDirectoryIfNotExists($@"{_settings.DataPath}\{_
settings.CommentsFolder}");
        }
        ...
        {
            var list = new List<T>();
            foreach (var f in Directory.GetFiles($@"{_settings.DataPath}\
{folder}"))
            {
...
json";
        ...
        {
            var list = await LoadAsync<BlogPost>(_settings.
            BlogPostsFolder);
            return list.Count;
        }
        ...
        {
            return await LoadAsync<Category>(_settings.
            CategoriesFolder);
        }
        public async Task<Category?> GetCategoryAsync(string id)
        {
            var list = await LoadAsync<Category>(_settings.
            CategoriesFolder);
        ...
        public async Task<List<Comment>> GetCommentsAsync(string
blogPostId)
        {
